...
Future<WalletAddress> _generateLtcWallet(String seedPhrase) async {
  ...
  final String address =
      P2WPKH(data: PaymentData(pubkey: keyPair.publicKey), network: litecoin)
              .data!
              .address ??
          "";
  return WalletAddress(
...
Future<WalletAddress> _generateKujiraWallet(String seedPhrase) async {
  ...
  final rootKey = bip32.BIP32.fromSeed(seedBytes);
  final derivedKey =
      rootKey.derivePath(_pathForKujiraPrivateKey); // BIP44 path for Kujira
...
Future<WalletAddress> _generateCosmosWallet(String seedPhrase) async {
  ...
  final rootKey = bip32.BIP32.fromSeed(seedBytes);
  final derivedKey =
      rootKey.derivePath(_pathForCosmosPrivateKey); // BIP44 path for Cosmos
  ...
  return WalletAddress(
    ...
    addressType: WalletAddressType
        .cosmos, // Assuming you have a type for Cosmos in WalletAddressType
    seedPhrase: seedPhrase,
  );
}

Future<WalletAddress> _generateThorchainWallet(String seedPhrase,
